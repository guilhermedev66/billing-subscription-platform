...
- **Idempotency-Key pattern, established in M3 on Subscriptions — reuse this shape for M4/M5 (webhooks, payments):** every mutating endpoint requires an `Idempotency-Key` header; the key + a hash of the normalized request + the response are persisted atomically (same transaction as the domain mutation) with a DB-unique constraint on `(organization_id, operation, idempotency_key)`. Same key + same body replays the stored response; same key + different body is 409. Never an in-memory cache.
- **A subscription's trial length lives on the `Price` (`TrialDays`), not on the create request** — `Subscription.Create` derives `Status`/`TrialEnd` from the chosen price's `TrialDays` at creation time. Proration on a `Trialing` subscription must always compute zero amount due (plan/seat still changes, just no charge) until `trial_end`.
- **Subscription lifecycle: Pause only from `Active`, Resume only from `Paused`→`Active`, and plan/seat changes are rejected while `Paused`** (must Resume first) — established in M3 after a QA finding that pausing a Trialing/PastDue/Unpaid subscription lost its original state on resume.
